...
public class FinalizeXmlTasklet implements Tasklet {
    ...
    public org.springframework.batch.repeat.RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        try (BufferedWriter bw = Files.newBufferedWriter(new FileSystemResource(outputFilePath).getFile().toPath(), StandardCharsets.UTF_8)) {
    ...
    private String readDetailsXmlRemovingProlog(String path) throws IOException {
        var lines = Files.readAllLines(new FileSystemResource(path).getFile().toPath(), StandardCharsets.UTF_8);
